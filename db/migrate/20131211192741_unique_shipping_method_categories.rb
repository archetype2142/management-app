class UniqueShippingMethodCategories < ActiveRecord::Migration[4.2]
  def change
    ...
    add_index klass.table_name, columns, unique: true, name: 'unique_shipping_method_categories'

    say "Remove redundant simple index on #{klass.table_name}"
    remove_index klass.table_name, name: 'index_shipping_method_categories_on_shipping_category_id'
  end
end
