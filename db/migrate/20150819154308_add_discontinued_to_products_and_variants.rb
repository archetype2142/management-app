class AddDiscontinuedToProductsAndVariants < ActiveRecord::Migration[4.2]
  def up
    add_column :products, :discontinue_on, :datetime, after: :available_on
    add_column :variants, :discontinue_on, :datetime, after: :deleted_at

    add_index :products, :discontinue_on
    add_index :variants, :discontinue_on
...
We will print out a report of the data we are fixing now: "
    ...
    Product.only_deleted.each do |product|
      # determine if there is a slug duplicate
      the_dup = Product.find_by(slug: product.slug)
      if the_dup.nil?
        # check to see if there are line items attached to any variants
        if Variant.with_deleted.where(product_id: product.id).map(&:line_items).any?
    ...
    Variant.only_deleted.each do |variant|
      # determine if there is a slug duplicate
      the_dup = Variant.find_by(sku: variant.sku)
      if the_dup.nil?
        # check to see if there are line items attached to any variants
  ...
  def down
    execute "UPDATE products SET deleted_at = discontinue_on WHERE deleted_at IS NULL"
    execute "UPDATE variants SET deleted_at = discontinue_on WHERE deleted_at IS NULL"

    remove_column :products, :discontinue_on
    remove_column :variants, :discontinue_on
  end
end
