class AddShippingCategoryToShippingMethodsAndProducts < ActiveRecord::Migration[4.2]
  def up
    default_category = ShippingCategory.first
    default_category ||= ShippingCategory.create!(name: "Default")

    ShippingMethod.all.each do |method|
      method.shipping_categories << default_category if method.shipping_categories.blank?
    end

    Product.where(shipping_category_id: nil).update_all(shipping_category_id: default_category.id)
  end
