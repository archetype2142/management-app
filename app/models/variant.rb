  class Variant < ApplicationRecord
    ...
    has_many :option_value_variants, class_name: 'OptionValueVariant'
    has_many :option_values, through: :option_value_variants, class_name: 'OptionValue'
    ...
    scope :backorderable, -> { joins(:stock_items).where(stock_items: { backorderable: true }) }
    ...
    scope :for_currency_and_available_price_amount, ->(currency = nil) do
      currency ||= 'PLN'
      joins(:prices).where('prices.currency = ?', currency).where('prices.amount IS NOT NULL').distinct
    end
    ...
    LOCALIZED_NUMBERS.each do |m|
      define_method("#{m}=") do |argument|
        self[m] = LocalizedNumber.parse(argument) if argument.present?
      end
    end
    ...
    def options_text
      Variants::OptionsPresenter.new(self).to_sentence
    end
    ...
    def set_cost_currency
      self.cost_currency = 'PLN' if cost_currency.blank?
    end
