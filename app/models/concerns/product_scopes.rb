  ...
  included do
    ...
    add_search_scope :in_taxon do |taxon|
      includes(:classifications).
        where('products_taxons.taxon_id' => taxon.self_and_descendants.pluck(:id)).
        order('products_taxons.position ASC')
    end
    ...
    add_search_scope :with_option_value do |option, value|
      ...
      group('products.id').joins(variants_including_master: :option_values).where(conditions)
    end
    ...
    add_search_scope :taxons_name_eq do |name|
      group('products.id').joins(:taxons).where(Taxon.arel_table[:name].eq(name))
    end
