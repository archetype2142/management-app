  class InventoryUnit < ApplicationRecord
    with_options inverse_of: :inventory_units do
      ...
      belongs_to :shipment, class_name: 'Shipment', touch: true, optional: true
      # has_many :return_items
      # has_many :return_authorizations, class_name: 'ReturnAuthorization', through: :return_items
      belongs_to :line_item, class_name: 'LineItem'
    end

    belongs_to :original_return_item, class_name: 'ReturnItem'
    ...
    def find_stock_item
      StockItem.where(stock_location_id: shipment.stock_location_id,
    ...
    def current_or_new_return_item
      ReturnItem.from_inventory_unit(self)
    end
