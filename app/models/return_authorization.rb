class ReturnAuthorization < ApplicationRecord
  # include Core::NumberGenerator.new(prefix: 'RA', length: 9)

  belongs_to :order, class_name: 'Order', inverse_of: :return_authorizations
  ...
  belongs_to :reason, class_name: 'ReturnAuthorizationReason', foreign_key: :return_authorization_reason_id
  ...
  def must_have_shipped_units
    if order.nil? || order.inventory_units.shipped.none?
      errors.add(:order, I18n.t(:has_no_shipped_units))
    end
  end
  ...
  def generate_expedited_exchange_reimbursements
    return unless Config[:expedited_exchanges]
