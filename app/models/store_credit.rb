  class StoreCredit < ApplicationRecord
    ...
    DEFAULT_CREATED_BY_EMAIL = 'management_admin@example.com'.freeze

    belongs_to :user, class_name: 'User', foreign_key: 'user_id'
    belongs_to :category, class_name: 'StoreCreditCategory'
    belongs_to :created_by, class_name: 'User', foreign_key: 'created_by_id'
    belongs_to :credit_type, class_name: 'StoreCreditType', foreign_key: 'type_id'
    ...
    scope :order_by_priority, -> { includes(:credit_type).order('store_credit_types.priority ASC') }
    ...
    def authorize(amount, order_currency, options = {})
      ...
      if validate_authorization(amount, order_currency)
        ...
        errors.add(:base, I18n.t('store_credit_payment_method.insufficient_authorized_amount'))
        false
      end
    end
    ...
    def validate_authorization(amount, order_currency)
      if amount_remaining.to_d < amount.to_d
        errors.add(:base, I18n.t('store_credit_payment_method.insufficient_funds'))
      elsif currency != order_currency
        errors.add(:base, I18n.t('store_credit_payment_method.currency_mismatch'))
      end
      errors.blank?
    end
    ...
    def void(authorization_code, options = {})
      if auth_event = store_credit_events.find_by(action: AUTHORIZE_ACTION, authorization_code: authorization_code)
        ...
        errors.add(:base, I18n.t('store_credit_payment_method.unable_to_void', auth_code: authorization_code))
        false
      end
    end
    ...
    def credit(amount, authorization_code, order_currency, options = {})
      ...
      if currency != order_currency # sanity check to make sure the order currency hasn't changed since the auth
        ...
        errors.add(:base, I18n.t('store_credit_payment_method.unable_to_credit', auth_code: authorization_code))
        false
      end
    end
    ...
    class << self
      def default_created_by
        User.find_by(email: DEFAULT_CREATED_BY_EMAIL)
      end
    end
    ...
    def create_credit_record(amount, action_attributes = {})
      ...
      credit = if Config.credit_to_new_allocation
                 StoreCredit.new(create_credit_record_params(amount))
               else
    ...
    def associate_credit_type
      unless type_id
        ...
        self.credit_type = StoreCreditType.find_by(name: credit_type_name)
      end
    end
  end
