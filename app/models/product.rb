class Product < ApplicationRecord
  ...
  has_many :promotion_rules, through: :product_promotion_rules, class_name: 'PromotionRule'

  has_many :promotions, through: :promotion_rules, class_name: 'Promotion'
  ...
  has_many :prices, -> { order('variants.position, variants.id, currency') }, through: :variants
  ...
  def default_variant
    ...
    Rails.cache.fetch("default-variant/#{cache_key_with_version}/#{track_inventory}") do
  ...
  def property(property_name)
    product_properties.joins(:property).find_by(properties: { name: property_name }).try(:value)
  end
  ...
  def brand
    taxons.joins(:taxonomy).find_by(taxonomies: { name: I18n.t(:taxonomy_brands_name) })
  end

  def category
    taxons.joins(:taxonomy).
      where(taxonomies: { name: I18n.t(:taxonomy_categories_name) }).
      order(depth: :desc).
      first
  end
  ...
  def touch_taxons
    Taxon.where(id: taxon_and_ancestors.map(&:id)).update_all(updated_at: Time.current)
    Taxonomy.where(id: taxonomy_ids).update_all(updated_at: Time.current)
  end
