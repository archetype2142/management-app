class ReturnItem < ApplicationRecord
  ...
  has_many :exchange_inventory_units, class_name: 'InventoryUnit',
                                      foreign_key: :original_return_item_id,
                                      inverse_of: :original_return_item
  belongs_to :exchange_variant, class_name: 'Variant'
  belongs_to :preferred_reimbursement_type, class_name: 'ReimbursementType'
  belongs_to :override_reimbursement_type, class_name: 'ReimbursementType'
  ...
  scope :exchange_required, -> { eager_load(:exchange_inventory_units).where(inventory_units: { original_return_item_id: nil }).distinct }
  ...
  def currency
    return_authorization.try(:currency) || Config[:currency]
  end
  ...
  def stock_item
    ...
    StockItem.find_by(variant_id: inventory_unit.variant_id,
  ...
  def belongs_to_same_customer_order
    ...
    if customer_return.order_id != inventory_unit.order_id
      errors.add(:base, I18n.t(:return_items_cannot_be_associated_with_multiple_orders))
    end
  end
  ...
  def eligible_exchange_variant
    ...
    unless eligible_exchange_variants.include?(exchange_variant)
      errors.add(:base, I18n.t(:invalid_exchange_variant))
    end
  end
  ...
  def sufficient_quantity_for_return
    ...
    errors.add(:return_quantity, I18n.t(:cannot_return_more_than_bought_quantity))
  end
  ...
  def validate_no_other_completed_return_items
    other_return_item = ReturnItem.where(inventory_unit_id: inventory_unit_id,
  ...
  def cancel_others
    ReturnItem.where(inventory_unit_id: inventory_unit_id).where.not(id: id).
      not_cancelled.each(&:cancel!)
  end

  def should_restock?
    resellable? && variant.should_track_inventory? && stock_item && Config[:restock_inventory]
  end
end
