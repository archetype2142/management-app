...
class Payment < ApplicationRecord
  # include Core::NumberGenerator.new(prefix: 'P', letters: true, length: 7)
  ...
  has_many :offsets, -> { offset_payment }, class_name: 'Payment', foreign_key: :source_id
  has_many :log_entries, as: :source
  has_many :state_changes, as: :stateful
  has_many :capture_events, class_name: 'PaymentCaptureEvent'
  ...
  scope :from_credit_card, -> { where(source_type: 'CreditCard') }
  scope :with_state, ->(s) { where(state: s.to_s) }
  # "offset" is reserved by activerecord
  scope :offset_payment, -> { where("source_type = 'Payment' AND amount < 0 AND state = 'completed'") }
  ...
  scope :store_credits, -> { where(source_type: StoreCredit.to_s) }
  scope :not_store_credits, -> { where(arel_table[:source_type].not_eq(StoreCredit.to_s).or(arel_table[:source_type].eq(nil))) }
  ...
  def validate_source
    if source && !source.valid?
      source.errors.each do |field, error|
        ...
        errors.add(I18n.t(source.class.to_s.demodulize.underscore), "#{field_name} #{error}")
      end
    end
    !errors.present?
  end
  ...
  def create_eligible_credit_event
    ...
    # payment has a source of itself (Payment) no matter the
    # type of payment getting refunded, hence the additional check
    # if the source is a store credit.
    return unless store_credit? && source.is_a?(StoreCredit)

    # creates the store credit event
    source.update!(action: StoreCredit::ELIGIBLE_ACTION,
