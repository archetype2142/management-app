...
class Adjustment < ApplicationRecord
  ...
  belongs_to :order, class_name: 'Order', inverse_of: :all_adjustments
  ...
  self.competing_promos_source_types = ['PromotionAction']

  scope :not_finalized, -> { where(state: 'open') }
  scope :finalized, -> { where(state: 'closed') }
  scope :tax, -> { where(source_type: 'TaxRate') }
  scope :non_tax, -> do
    source_type = arel_table[:source_type]
    where(source_type.not_eq('TaxRate').or(source_type.eq(nil)))
  end
  scope :price, -> { where(adjustable_type: 'LineItem') }
  scope :shipping, -> { where(adjustable_type: 'Shipment') }
  scope :optional, -> { where(mandatory: false) }
  scope :eligible, -> { where(eligible: true) }
  scope :charge, -> { where("#{quoted_table_name}.amount >= 0") }
  scope :credit, -> { where("#{quoted_table_name}.amount < 0") }
  scope :nonzero, -> { where("#{quoted_table_name}.amount != 0") }
  scope :promotion, -> { where(source_type: 'PromotionAction') }
  scope :return_authorization, -> { where(source_type: 'ReturnAuthorization') }
  scope :is_included, -> { where(included: true) }
  scope :additional, -> { where(included: false) }
  scope :competing_promos, -> { where(source_type: competing_promos_source_types) }
  scope :for_complete_order, -> { joins(:order).merge(Order.complete) }
  scope :for_incomplete_order, -> { joins(:order).merge(Order.incomplete) }

  extend DisplayMoney
  money_methods :amount

  def amount=(amount)
    self[:amount] = LocalizedNumber.parse(amount)
  end
